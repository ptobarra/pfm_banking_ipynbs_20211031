...
from pandas import read_excel, date_range, DataFrame, to_numeric, to_datetime
...
'requirements_20210616_seguro_medico_prophet_ide.txt'.
"""
...
transacciones_90_df = transacciones_90_df.rename(columns={'Fecha transacción': 'FECHA', 'Importe': 'IMPORTE'},
                                                 inplace=False)
...
# miraremos tres meses más atrás del mes objetivo para ver si hubo recibos en esos dos meses y no nos hemos dado de
# baja del servicio (estamos mirando desde 2 meses más atrás del mes que pedimos la predicción)
...
for d in range(int((stat_mode_obj - limInfRecibo_obj).days)):
    # estas lineas no se ejecutan en producción ### fecha_str = str((limInfRecibo_obj + timedelta(days=d)).year) +
    # '-' + str((limInfRecibo_obj + timedelta(days=d)).month) + '-' + str((limInfRecibo_obj +
    # timedelta(days=d)).day) print(fecha_str) ###
    if (limInfRecibo_obj + timedelta(days=d)) in transacciones_90_dia_df.index:
        num_recibos += 1

# hacemos el dataframe de la serie temporal del importe de los recibos y estimados el valor del recibo para el mes
# siguiente a la fecha en la que pedimos la predicción
...
# Vamos a hacer una predicción desde el 1er dia en que tenemos datos hasta el ultimo dia
